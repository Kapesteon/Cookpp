...
void Menu::putInShoppingList(Aliment aliment, double newMass) {
	...
	if (alimentInShopingList == false) {
		InfoNutri * infoN = new InfoNutri;
		*infoN = aliment.getInfoNutri();
		Aliment alimentToAdd = Aliment(aliment.getName(), aliment.getType(), aliment.getSeason(), infoN, newMass);
